...
if __name__ == '__main__':
  ...
  engine = create_engine('mysql://root:0000@localhost/portfolio_values')
  current_portfolio = pd.read_sql_table('satellite_portfolio', con = engine,
  ...
  for i in range(len(fund_combinations.index)):
    ...
    if set(my_list) == set(list(zip(*current_portfolio.values.tolist()))[0]):
        ...
        if difference > (threshold * final_portfolios.iloc[i,-1]):
            ...
            current_portfolio.to_sql('satellite_portfolio', con=engine,if_exists = 'replace')
  ...
  pd.DataFrame(final_portfolios.iloc[0,:].values,index = list(pd.read_sql_table('top_satellite_portfolios', con = engine,
                schema = 'portfolio_values').columns[1:]), columns = [prices.index[-1]]).transpose().to_sql('top_satellite_portfolios', con=engine,if_exists = 'append')
  pd.DataFrame(feature_imp, index = list(pd.read_sql_table('satellite_features', con = engine,
                schema = 'portfolio_values').columns[1:]), columns = [prices.index[-1]]).transpose().to_sql('satellite_features', con=engine,if_exists = 'append')